...
//     const errs = [];
//     if (!email) errs.push("Official email is required.");
//     if (!password) errs.push("Password is required.");
//     if (errs.length) return setErrors(errs);

//     const payload = {
//       // still called "email" so backend continues to work
//       email: String(email).trim().toLowerCase(),
//       password: String(password).trim(),
//       remember,
//     };
...
//       // Prefer AuthContext.login if available
//       if (typeof ctxLogin === "function") {
//         const u = await ctxLogin(
//           payload.email,
//           payload.password,
//           payload.remember
//         );
...
//         // If backend flags must_change_password, force redirect
...
//       if (!u) {
//         throw new Error(res?.data?.message || "Login response missing user.");
//       }
...
//           {/* Remove this block if users are not allowed to self-register */}
//           <p className="text-sm text-gray-600 text-center">
//             Don’t have an account?{" "}
//             <Link to="/register" className="text-customRed font-medium hover:underline">
//               Sign up
...
export default function Login({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // fetch CSRF token + set interceptor (safe even if backend ignores it)
      await initCsrf();

      // Prefer AuthContext.login if available
      if (typeof ctxLogin === "function") {
        // ctxLogin should handle token + user state
      ...
      // NOTE: backend route is now /auth/login
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              {/* For now, password resets are admin-driven, so hide this link.
                  Keep it if you plan to add a user-facing reset flow later. */}
              {/* <Link to="/forgot-password" className="text-customRed hover:underline">
                Forget Password?
              </Link> */}
            </div>
          ...
          {/* Registration is disabled – employees are created by admin/HR */}
          {/* <p className="text-sm text-gray-600 text-center">
            Don’t have an account?{" "}
            <Link to="/register" className="text-customRed font-medium hover:underline">
              Sign up
