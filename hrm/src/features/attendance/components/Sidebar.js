      ...
      <div className="sidebar-card">
        ...
        <div className="sidebar-list">
          {items.map((it) => {
            ...
            const iconKey = it.icon || it.id || "employee-list";
            ...
            return (
              <button
                key={it.id}
                onClick={() => !isWorking && onNavigate?.(it.id)}
                className={`sidebar-item ${active ? "sidebar-active" : ""} ${isWorking ? "text-slate-400 cursor-not-allowed opacity-75" : ""}`}
                type="button"
              >
                ...
                <span className="shrink-0">
                  {React.cloneElement(icons[iconKey] || icons["employee-list"], {
                    className: `sidebar-icon ${active ? "text-customRed" : isWorking ? "text-slate-300" : "text-gray-400"}`,
                  })}
                </span>

                {/* Label */}
                <span className="truncate pr-1">{it.label}</span>
                {isWorking && badge}
              </button>
            );
          })}
